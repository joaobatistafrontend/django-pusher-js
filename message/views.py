...
class IndexView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if 'pausa' in request.GET:
            print('pausou aqui')            
            self.pusher_client.trigger('painel', 'pausa', {'message': 'pausou'})
               
        if 'parar' in request.GET:
            print('pausou aqui')            
            self.pusher_client.trigger('painel', 'parar', {'message': 'parou'})

        if 'um' in request.GET:
            print('um aqui')            
            self.pusher_client.trigger('painel', 'um', {'message': 'um'})
        
        if 'dois' in request.GET:
            print('dois aqui')            
            self.pusher_client.trigger('painel', 'dois', {'message': 'dois'})
        
        if 'tres' in request.GET:
            print('tres aqui')             
            self.pusher_client.trigger('painel', 'tres', {'message': 'tres'})
        
        if 'quatro' in request.GET:
            print('quatro aqui')            
            self.pusher_client.trigger('painel', 'quatro', {'message': 'quatro'})

        if 'cinco' in request.GET:
            print('cinco aqui')            
            self.pusher_client.trigger('painel', 'cinco', {'message': 'cinco'})
